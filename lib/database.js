...
module.exports = {
    ...
    grupoInfoAntiFlood: (id_grupo) => {
        let antifloodJson = JSON.parse(fs.readFileSync(path.resolve('database/json/antiflood.json'))),
            grupoIndex = antifloodJson.findIndex(item => item.id_grupo == id_grupo)
    ...
    addMsgFlood: async(id_grupo, usuario_msg) => {
        try {
            let antifloodJson = JSON.parse(fs.readFileSync(path.resolve('database/json/antiflood.json'))),
                grupoIndex = antifloodJson.findIndex(item => item.id_grupo == id_grupo)
            let grupo_info = antifloodJson[grupoIndex],
                timestamp_atual = Math.round(new Date().getTime() / 1000),
                resposta = false
    ...
    obterUsuariosInativos: async(id_grupo, min) => {
        min = parseInt(min)
        let usuarios_inativos = await db.contador.asyncFind({ id_grupo, msg: { $lt: min } }, [
            ["sort", { msg: -1 }]
        ])
        return usuarios_inativos
    },
    obterUsuariosAtivos: async(id_grupo, limite) => {
        let usuarios_ativos = await db.contador.asyncFind({ id_grupo }, [
            ["sort", { msg: -1 }],
            ['limit', limite]
        ])
